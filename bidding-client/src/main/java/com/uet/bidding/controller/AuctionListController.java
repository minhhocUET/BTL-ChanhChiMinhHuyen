...
public class AuctionListController implements Initializable {
  ...
  @FXML
  private TableColumn<Auction, String> colItemType;
  @FXML
  private TableColumn<Auction, String> colProductName;
  @FXML
  private TableColumn<Auction, Integer> colRegistered;
    ...
    tableView.setRowFactory(tv -> {
      ...
      row.setOnMouseClicked(e -> {
        if (!row.isEmpty() && row.getItem() != null
            && row.getItem().getId() != -1 && e.getClickCount() >= 2) {
            ...
            btn.setOnAction(event -> {
              ...
              if (currentUser != null) {
                if (currentUser.getFullName() == null
                  ...
                  alert.setContentText("Vui lòng hoàn thiện TẤT CẢ thông tin "
                      + "để có thể tham gia đấu giá hoặc đăng bán.");
                  alert.showAndWait();
                  return;
                }
              }
              ...
              openAuctionDetail(selectedAuction,
  ...
  private void openProductDetail(Auction auction, Scene scene) {
    if (auction == null || scene == null) {
      return;
    }
  ...
  private void openAuctionDetail(Auction auction, Stage stage) {
    ...
    try {
      String itemName = auction.getItem() != null
          ? auction.getItem().getName() : ("#" + auction.getId());
      Parent root;
      ...
      if (isOwnAuction(auction)) {
        ...
        ownSessionAlert.setContentText("Bạn đã vào phiên của chính mình. "
            + "Chuyển sang giao diện người bán.");
        ownSessionAlert.showAndWait();

        SellerAuctionContext.set(auction);
        FXMLLoader loader = new FXMLLoader(
            getClass().getResource("/SellerProductDetail.fxml"));
        root = loader.load();
        ...
        .thenAccept(response -> Platform.runLater(() -> {
          try {
            if ("SUCCESS".equals(response.getType())) {
              String json = ClientService.getInstance().getGson()
                  .toJson(response.getData());
              List<Auction> list = ClientService.getInstance().getGson()
                  .fromJson(json, new TypeToken<List<Auction>>() { }.getType());

              if (list != null && !list.isEmpty()) {
                tableView.setItems(FXCollections.observableArrayList(list));
              } else {
                // 🌟 Nếu không có phiên nào, nạp 10 dòng ảo để hiện khung lưới
  ...
  public void handleAuctionBroadcast(Auction updated) {
    if (tableView == null || updated == null) {
      return;
    }
  ...
  public void refreshOneAuction(Auction updated) {
    ...
    if ("FINISHED".equals(updated.getStatus())) {
      ...
      // 🌟 Nếu sau khi xóa phiên kết thúc mà bảng trống trơn, nạp lại dòng ảo ngay
  ...
  public void addOrRefreshAuction(Auction auction) {
    if (tableView == null || auction == null) {
      return;
    }
    ...
    // 🌟 Trước khi thêm hàng thật mới, nếu có dòng ảo (id == -1) thì xóa sạch dòng ảo đi
